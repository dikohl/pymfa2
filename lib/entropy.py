...
class EntropyCalc(object):
    ...
    def run(self):
        ...
        for key in range(len(stages)):
            stageResult = self.computeSingleStage(stages[int(key) + 1])
    ...
    def getCalcValues(self, materialFlows):
        ...
        for flow in materialFlows:
            ...
            if '' not in flow[1:3]:
                ...
                concentrationValue = self.concentrationValues.get((srcName, targName), [srcName] + [targName] +
    ...
    def convertUnits(self, flowValue):
        #find the units of the source node and taget node
