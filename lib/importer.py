...
class CSVImporter(object):
    ...
    def checkForEntropy(self, row):
        if not self.haveEntropy:
            if row[0].lower().replace(" ","") == "entropy:":
                if len(row[1]) == 0:
                    pass
                elif row[1].lower().replace(" ", "") in ["0", "n", "no", "none"]:
                    pass
    ...
    def checkNumberOfColumns(self,row):
        if not self.system.timeIndices and len(row) < 10:
            raise CSVParserException(
        ...
        if self.system.timeIndices and len(row) != 8 + len(self.system.timeIndices):
            raise CSVParserException(
    ...
    def checkAndHandleData(self,row, metadata, description, values):
        ...
        (transferType,src,srcMaterial,srcUnit,dst,dstMaterial,dstUnit) = [x.lower() for x in metadata]
        # assign column numbers to the column headers
        (colTransferType,colSrc,colSrcMaterial,colSrcUnit,colDst,colDstMaterial,colDstUnit) = [0, 1, 2, 3, 4, 5, 6]
        ...
        self.createNodes(transferType,src,srcMaterial,srcUnit,dst,dstMaterial,dstUnit,
            colTransferType,colSrc,colSrcMaterial,colSrcUnit,colDst,colDstMaterial,colDstUnit, splittedValues, nodeName, targetName, metadata, description, values)
        return
    ...
    def createNodes(self, transferType,src,srcMaterial,srcUnit,dst,dstMaterial,dstUnit,colTransferType,colSrc,colSrcMaterial,colSrcUnit,colDst,colDstMaterial,colDstUnit, splittedValues, nodeName, targetName, metadata, description, values):
        ...
        if transferType == "inflow":
            ...
            if nodeName not in self.system.nodes:
                ...
                if (targetName in self.system.rates[nodeName].descriptions or targetName in self.system.rates[nodeName].transfers):
                    raise CSVParserException(
